...
export default function DMPanel({ conversation, onNewMessage, onViewProfile, onNavigateToGroup }) {
  ...
  const parsePrivateReply = (content) => {
    ...
    return {
      ...
      groupName:  r1.slice(0, c2).replace(/ ÂÂ·/g, ':'),
      senderName: r2.slice(0, c3).replace(/ ÂÂ·/g, ':'),
      ...
      <div style={{ padding: '12px 16px 16px', borderTop: '1px solid #2A2A36', flexShrink: 0, background: '#111116' }}>
        ...
        {replyTo && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, padding: '7px 12px', borderRadius: 8, borderLeft: '3px solid #6366F1', background: '#16161E' }}>
            ...
            <button onClick={() => setReplyTo(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#55556E', fontSize: 16, lineHeight: 1, flexShrink: 0 }}> ÂÂ·</button>
          </div>
        )}
        {/* Attached file pills */}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
            {attachedFiles.map(f => (
              <span key={f.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 8, background: '#1C1C26', border: '1px solid #2A2A36', fontSize: 11, color: '#9090B0' }}>
                ...
                <button onMouseDown={e => { e.preventDefault(); setAttachedFiles(prev => prev.filter(r => r.id !== f.id)); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#55556E', lineHeight: 1, padding: 0 }} onMouseEnter={e => e.currentTarget.style.color = 'rgba(239,68,68,0.7)'} onMouseLeave={e => e.currentTarget.style.color = '#55556E'}> ÂÂ·</button>
              </span>
            ))}
          </div>
        )}
