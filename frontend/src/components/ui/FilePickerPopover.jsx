...
/**
 * Popover that lists group files and lets the user pick one to insert as a reference.
 * onPick(markdownRef) — called with the markdown string to insert
 */
export default function FilePickerPopover({ groupId, onPick, onClose }) {
  ...
  const handlePick = (file) => {
    // Format: {{file:id:filename:url}} — safe, no markdown special chars
    const ref = `{{file:${file.id}:${file.filename}:${file.file_url}}}`;
    onPick(ref);
    onClose();
  };
        ...
        {loading ? (
          ...
          <button key={file.id} onClick={() => handlePick(file)}
            className="w-full flex items-center gap-2.5 px-3 py-2.5 text-left
              dark:hover:bg-gray-800 hover:bg-gray-50 transition">
