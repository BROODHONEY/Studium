  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <p style={{ fontSize: 11, fontWeight: 300, color: 'var(--text-3)', margin: '2px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {group.subject}
      ...
      {/* Desktop tabs */}
      <div className="hidden sm:flex" style={{ gap: 1, flexShrink: 0, background: 'var(--bg-raised)', borderRadius: 10, padding: '3px', border: '1px solid var(--border-color)' }}>
        {tabs.map(tab => {
      ...
      <button ref={triggerRef}
        ...
        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 400, border: '1px solid var(--border-color)', background: 'var(--bg-raised)', color: 'var(--text-1)', cursor: 'pointer', flexShrink: 0 }}>
        {activeTab}
      ...
      {dropdownOpen && createPortal(
        <div ref={menuRef}
          style={{ position: 'fixed', top: dropdownPos.top, right: dropdownPos.right, width: 160, zIndex: 9999, background: 'var(--bg-surface)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid var(--border-color)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
          {tabs.map(tab => (
            <button key={tab}
