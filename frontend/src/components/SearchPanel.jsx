...
const TAG_COLORS = {
  urgent: 'rgba(239,68,68,0.8)', exam: 'rgba(165,180,252,0.8)',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: 'Inter, sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '8px 12px 10px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-raised)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: 10, padding: '8px 12px', boxShadow: '0 0 0 1px rgba(99,102,241,0.08)' }}>
          ...
          {query && !loading && (
            <button onClick={clear} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', lineHeight: 0, padding: 0, flexShrink: 0 }}>
        ...
        <select value={groupId} onChange={e => handleGroup(e.target.value)}
          style={{ width: '100%', marginTop: 8, background: 'var(--bg-raised)', border: '1px solid var(--border-color)', borderRadius: 8, padding: '7px 10px', fontSize: 12, fontWeight: 300, color: groupId ? 'var(--text-1)' : 'var(--text-3)', outline: 'none', fontFamily: 'Inter, sans-serif', appearance: 'none', cursor: 'pointer' }}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 16px' }}>
        ...
        {results && total === 0 && (
          <p style={{ fontSize: 12, fontWeight: 300, color: 'var(--text-3)', textAlign: 'center', padding: '20px 8px' }}>No results for "{query}"</p>
        )}
        {results && total > 0 && (
          <>
            <p style={{ fontSize: 10, fontWeight: 300, color: 'var(--text-3)', padding: '4px 4px 10px', margin: 0 }}>{total} result{total !== 1 ? 's' : ''}</p>
            <SideSection title="Messages" items={results.messages}
              renderItem={msg => (
                <button key={msg.id} onClick={() => onNavigate?.({ type: 'message', groupId: msg.group_id, group: msg.group, messageId: msg.id })}
                  style={{ width: '100%', textAlign: 'left', padding: '8px 10px', borderRadius: 8, background: 'none', border: '1px solid var(--border-color)', cursor: 'pointer', transition: 'background 0.1s' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(99,102,241,0.05)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'none'}>
                  <p style={{ fontSize: 10, color: 'rgba(99,102,241,0.7)', margin: '0 0 2px' }}>{msg.group?.name}</p>
                  <p style={{ fontSize: 12, fontWeight: 300, color: 'var(--text-1)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{msg.content?.slice(0, 60)}</p>
                </button>
              )}
            />
            <SideSection title="Files" items={results.files}
              renderItem={file => (
                <button key={file.id} onClick={() => onNavigate?.({ type: 'file', groupId: file.group_id, group: file.group, fileId: file.id })}
                  style={{ width: '100%', textAlign: 'left', padding: '8px 10px', borderRadius: 8, background: 'none', border: '1px solid var(--border-color)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, transition: 'background 0.1s' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(99,102,241,0.05)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'none'}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-1)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.filename}</p>
                    <p style={{ fontSize: 10, color: 'var(--text-3)', margin: '1px 0 0' }}>{file.group?.name}</p>
                  </div>
                </button>
              )}
            />
            <SideSection title="Announcements" items={results.announcements}
              renderItem={ann => (
                <button key={ann.id} onClick={() => onNavigate?.({ type: 'announcement', groupId: ann.group_id, group: ann.group })}
                  ...
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(99,102,241,0.05)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'none'}>
                  <p style={{ fontSize: 10, color: 'rgba(99,102,241,0.7)', margin: '0 0 2px' }}>{ann.group?.name}</p>
                  <p style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-1)', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ann.title}</p>
                  <p style={{ fontSize: 11, fontWeight: 300, color: 'var(--text-3)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ann.content?.slice(0, 50)}</p>
                </button>
              )}
            />
          </>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', textAlign: 'center', padding: '0 32px' }}>
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'rgba(99,102,241,0.4)', margin: '0 auto 14px', display: 'block' }}>
          <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
        </svg>
        <p style={{ fontSize: 15, fontWeight: 400, color: 'var(--text-2)', margin: '0 0 8px' }}>Search</p>
        <p style={{ fontSize: 13, fontWeight: 300, color: 'var(--text-3)', margin: 0, lineHeight: 1.6 }}>
