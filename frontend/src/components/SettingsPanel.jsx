...
// -- Multi-item list editor (with file attachments + date) --
function ListEditor({ label, accentColor, accentLo, icon, items, onChange, titlePlaceholder, subtitlePlaceholder, showDate }) {
  const [adding, setAdding] = useState(false);
  const [draft, setDraft] = useState({ title: '', subtitle: '', date: '', attachments: [] });
  ...
  const handleFileAdd = async (e) => {
    ...
    try {
      ...
      setDraft(d => ({ ...d, attachments: [...d.attachments, { name: res.data.name, url: res.data.url, type: res.data.type }] }));
    } catch (err) {
  ...
  const removeAttachment = (ai) => setDraft(d => ({ ...d, attachments: d.attachments.filter((_, i) => i !== ai) }));
  ...
  const commit = () => {
    ...
    setDraft({ title: '', subtitle: '', date: '', attachments: [] });
    setAdding(false);
    setUploadError('');
  };
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {items.length > 0 && (
            <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 20, background: accentLo, color: accentColor, border: `1px solid ${accentColor}30` }}>{items.length}</span>
          )}
        </div>
        <button onClick={() => { setAdding(v => !v); setUploadError(''); }}
          style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 8, background: adding ? accentLo : 'none', border: `1px solid ${adding ? accentColor + '50' : T.border}`, color: adding ? accentColor : T.text3, fontSize: 11, fontWeight: 500, cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'all 0.15s' }}>
      ...
      {/* Add form */}
      {adding && (
        <div style={{ background: T.surface, border: `1px solid ${accentColor}35`, borderRadius: 12, padding: '14px', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <input value={draft.title} onChange={e => setDraft(d => ({ ...d, title: e.target.value }))}
            placeholder={titlePlaceholder || 'Title'} style={inp}
            onFocus={e => e.target.style.borderColor = accentColor}
            onBlur={e => e.target.style.borderColor = T.border}
            onKeyDown={e => { if (e.key === 'Escape') setAdding(false); }} />
          <input value={draft.subtitle} onChange={e => setDraft(d => ({ ...d, subtitle: e.target.value }))}
            placeholder={subtitlePlaceholder || 'Details (optional)'} style={inp}
            onFocus={e => e.target.style.borderColor = accentColor}
            onBlur={e => e.target.style.borderColor = T.border}
            onKeyDown={e => { if (e.key === 'Escape') setAdding(false); }} />
          ...
          {showDate && (
            <div>
              ...
              <input type="month" value={draft.date} onChange={e => setDraft(d => ({ ...d, date: e.target.value }))}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            {draft.attachments.map((att, ai) => (
              <div key={ai} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px 4px 6px', borderRadius: 8, background: T.card, border: `1px solid ${T.border}`, fontSize: 11, color: T.text2, maxWidth: 200 }}>
                {isImage(att.type)
                ...
                <button onClick={() => removeAttachment(ai)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.text3, lineHeight: 0, padding: 0, flexShrink: 0 }}
          ...
          {uploadError && <p style={{ fontSize: 11, color: T.danger, margin: 0 }}>{uploadError}</p>}

          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={commit} disabled={!draft.title.trim()}
              style={{ flex: 1, padding: '9px', borderRadius: 9, background: draft.title.trim() ? accentColor : T.card, border: 'none', color: draft.title.trim() ? '#131313' : T.text3, fontSize: 12, fontWeight: 600, cursor: draft.title.trim() ? 'pointer' : 'not-allowed', transition: 'all 0.15s' }}>
              Add
            </button>
            <button onClick={() => { setAdding(false); setDraft({ title: '', subtitle: '', date: '', attachments: [] }); setUploadError(''); }}
              style={{ flex: 1, padding: '9px', borderRadius: 9, background: 'none', border: `1px solid ${T.border}`, color: T.text2, fontSize: 12, cursor: 'pointer' }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {items.length === 0 && !adding ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {items.map((item, i) => (
            <div key={i} style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, borderLeft: `3px solid ${accentColor}`, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px' }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: accentLo, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: accentColor }}>
                  {icon}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {item.date && <p style={{ fontSize: 10, fontWeight: 500, color: accentColor, margin: '4px 0 0' }}>{fmtDate(item.date)}</p>}
                </div>
                <button onClick={() => remove(i)}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.text3, lineHeight: 0, padding: 2, flexShrink: 0, transition: 'color 0.15s' }}
                  onMouseEnter={e => e.currentTarget.style.color = T.danger}
                  onMouseLeave={e => e.currentTarget.style.color = T.text3}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {isStudent && (
        <>
          <div style={card}>
            <ListEditor label="Achievements" accentColor={T.primary} accentLo={T.primaryLo} showDate
              ...
              items={achievements} onChange={v => { setAchievements(v); markListEdited(); }}
              titlePlaceholder="e.g. Dean's List, Hackathon Winner" subtitlePlaceholder="e.g. Academic Excellence 2024" />
          </div>
          <div style={card}>
            <ListEditor label="Internships" accentColor={T.secondary} accentLo={T.secondaryLo} showDate
              ...
              items={internships} onChange={v => { setInternships(v); markListEdited(); }}
              titlePlaceholder="e.g. Software Intern @ Google" subtitlePlaceholder="e.g. Summer 2024 · 3 months" />
          </div>
          <div style={card}>
            <ListEditor label="Certificates" accentColor={T.green} accentLo={T.greenLo} showDate
              ...
              items={certificates} onChange={v => { setCertificates(v); markListEdited(); }}
              titlePlaceholder="e.g. AWS Certified, Google Cloud" subtitlePlaceholder="e.g. Issued by Amazon · 2024" />
          </div>
        </>
      )}
