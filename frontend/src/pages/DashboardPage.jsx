...
const NAV_MAIN = ['groups', 'dms', 'search', 'notifications'];
const NAV_ALL  = ['groups', 'dms', 'search', 'notifications', 'settings'];
...
function SideNavItem({ id, active, onClick, badge }) {
  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        background: active ? 'linear-gradient(135deg,rgba(124,58,237,0.15),rgba(37,99,235,0.08))' : 'none',
    ...
    >
      ...
      <span style={{ fontSize: 13, fontWeight: active ? 500 : 300, flex: 1 }}>{label}</span>
      {badge > 0 && (
    ...
    >
      ...
      <div className="hidden sm:flex" style={{ height: '100dvh', fontFamily: 'Inter, sans-serif', overflow: 'hidden', backgroundColor: 'var(--bg-void)', flexDirection: 'column' }}>
        ...
        <div className="dash-topbar t-divider" style={{ height: 52, flexShrink: 0, borderBottomWidth: 1, borderBottomStyle: 'solid', display: 'flex', alignItems: 'center', paddingRight: 20 }}>
          ...
          <div style={{ width: 220, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10, padding: '0 16px' }}>
            <img src={logo} alt="logo" style={{ width: 26, height: 26, objectFit: 'contain', flexShrink: 0 }} />
            <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-1)', letterSpacing: '-0.02em' }}>Studi+</span>
          </div>
          {/* Spacer */}
          <div style={{ flex: 1 }} />
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
          ...
          {/* ── Left sidebar ── */}
          <div className="dash-sidebar" style={{ width: 220, flexShrink: 0, borderRightWidth: 1, borderRightStyle: 'solid', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

            {/* Main nav */}
            <div style={{ padding: '10px 8px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
              {NAV_MAIN.map(id => (
                <SideNavItem key={id} id={id} active={activeNav === id} onClick={() => setActiveNav(id)} />
              ))}
            </div>

            {/* Groups FAB — only shown when groups tab active */}
            {activeNav === 'groups' && (
              <div style={{ padding: '0 8px 8px', flexShrink: 0 }}>
                <div style={{ position: 'relative' }}>
                  <button onClick={() => setFabOpen(v => !v)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(124,58,237,0.25)', background: fabOpen ? 'rgba(124,58,237,0.15)' : 'rgba(124,58,237,0.08)', color: '#7c3aed', cursor: 'pointer', fontSize: 12, fontWeight: 400, fontFamily: 'Inter, sans-serif', transition: 'background 0.15s' }}>
                    <svg width="11" height="11" viewBox="0 0 16 16" fill="currentColor"><path d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2z"/></svg>
                    New group / folder
                  </button>
                  {fabOpen && (
                    <>
                      <div style={{ position: 'fixed', inset: 0, zIndex: 998 }} onClick={() => setFabOpen(false)} />
                      <div style={{ position: 'absolute', top: 36, left: 0, right: 0, zIndex: 999, background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: 10, overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.15)' }}>
                        <button onClick={() => { setFabOpen(false); setNewFolderOpen(true); }}
                          ...
                          <svg width="13" height="13" viewBox="0 0 16 16" fill="currentColor" style={{ color: 'var(--text-3)' }}>
            ...
            <div className="t-divider" style={{ borderTopWidth: 1, borderTopStyle: 'solid', padding: '8px 8px', flexShrink: 0 }}>
              <button
                style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'none', fontFamily: 'Inter, sans-serif', color: 'var(--text-3)', transition: 'background 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(124,58,237,0.06)'; e.currentTarget.style.color = 'var(--text-2)'; }}
